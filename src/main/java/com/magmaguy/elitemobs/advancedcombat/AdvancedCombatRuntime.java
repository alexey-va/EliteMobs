...
public final class AdvancedCombatRuntime implements Listener, PlayerCombatState.Listener {
    ...
    private final AdvancedCombatShutdownPolicy shutdownPolicy = new AdvancedCombatShutdownPolicy();
    ...
    public AdvancedCombatRuntime(PlayerCombatState combatState) {
        if (instance != null)
    ...
    public static boolean isActive(Player player) {
        return instance != null && AdvancedCombatSystemConfig.isEnabled()
    ...
    public static void clearHudProvider() {
        hudProvider = AdvancedCombatRuntime::defaultHud;
    }
    ...
    public void start() {
        ...
        reconciliationTask = Bukkit.getScheduler().runTaskTimer(
                ...
                1L,
                AdvancedCombatRules.RECONCILIATION_INTERVAL_TICKS);
    ...
    public void shutdown() {
        ...
        boolean preserveHealth = shutdownPolicy.consumeHealthCleanup()
                == AdvancedCombatShutdownPolicy.HealthCleanup.PRESERVE_FOR_SOFT_RELOAD;
    ...
    private void reconcilePlayer(Player player, boolean enteredNewWorld) {
        ...
        boolean shouldBeActive = AdvancedCombatSystemConfig.isEnabled()
                && DungeonCombatRuntime.isEligiblePlayer(player);
        boolean isActive = activePlayers.containsKey(player.getUniqueId());
        if (isActive && player.isDead() && AdvancedCombatSystemConfig.isEnabled()
    ...
    private void enter(Player player) {
        ...
        HealthDisplayCoordinator.acquire(player, HealthDisplayCoordinator.Owner.ADVANCED_COMBAT);
        ...
        if (!combatState.isInCombat(player.getUniqueId())) {
            ...
            if (maxHealth != null && player.getHealth() < maxHealth.getValue()) {
                double healing = maxHealth.getValue()
                        * AdvancedCombatRules.OUT_OF_COMBAT_HEAL_FRACTION_PER_SECOND;
    ...
    private void leave(Player player, boolean preserveHealth) {
        ...
        HealthDisplayCoordinator.release(player, HealthDisplayCoordinator.Owner.ADVANCED_COMBAT);
    ...
    public static void clearPersistedState(Player player) {
        ...
        HealthDisplayCoordinator.release(player, HealthDisplayCoordinator.Owner.ADVANCED_COMBAT);
        HealthDisplayCoordinator.clearStaleAdvancedDisplay(player);
        ...
        player.setFoodLevel(AdvancedCombatRules.DISPLAY_FOOD_LEVEL);
        ...
        HealthPercentagePreserver.during(player, maxHealth, () -> {
            ...
            if (shouldExist) {
                maxHealth.addModifier(new AttributeModifier(
                        modifierKey,
                        AdvancedCombatRules.maxHealthFlatIncrease(),
    ...
    private static void maintainHunger(Player player) {
        if (player.getFoodLevel() != AdvancedCombatRules.SERVER_FOOD_LEVEL)
            player.setFoodLevel(AdvancedCombatRules.SERVER_FOOD_LEVEL);
        ...
        player.sendHealthUpdate(
                displayedHealth,
                AdvancedCombatRules.DISPLAY_FOOD_LEVEL,
                0F);
    }
    ...
    public void onFoodLevelChange(FoodLevelChangeEvent event) {
        ...
        event.setFoodLevel(AdvancedCombatRules.SERVER_FOOD_LEVEL);
    ...
    public void onFoodConsumed(PlayerItemConsumeEvent event) {
        ...
        Bukkit.getScheduler().runTask(MetadataHandler.PLUGIN, () -> {
            ...
            AttributeInstance maxHealth = player.getAttribute(Attribute.MAX_HEALTH);
            if (maxHealth == null) return;
            double healing = saturation * AdvancedCombatRules.FOOD_SATURATION_TO_HEALTH;
    ...
    public void onCakeConsumed(PlayerInteractEvent event) {
        ...
        Bukkit.getScheduler().runTask(MetadataHandler.PLUGIN, () -> {
            ...
            applyHealing(
                    player,
                    AdvancedCombatRules.CAKE_SLICE_SATURATION
                            * AdvancedCombatRules.FOOD_SATURATION_TO_HEALTH,
    ...
    static double saturationOf(ItemStack itemStack) {
        return AdvancedFoodItems.saturationOf(itemStack);
    }

    private static boolean isFood(ItemStack itemStack) {
        return AdvancedFoodItems.isFood(itemStack);
    }
    ...
    public void onPlayerChangedWorld(PlayerChangedWorldEvent event) {
        ...
        if (!DungeonCombatRuntime.isEligiblePlayer(player)) {
            ...
            else AdvancedCombatStateRecovery.reconcile(player);
            return;
        }
        Bukkit.getScheduler().runTask(MetadataHandler.PLUGIN, () -> {
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        activePlayers.put(player.getUniqueId(), new PlayerBaseline(
                AdvancedCombatRules.DISPLAY_FOOD_LEVEL, 5F, 0F));
    }
