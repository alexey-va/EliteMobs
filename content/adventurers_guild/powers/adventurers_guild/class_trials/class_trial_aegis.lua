...
return T.encounter{
 ...
 choose=function(c,s)
  if s.phasePending and T.ready(s,'steed') then
   s.phasePending=false; local seq=P.rescue(c,s,{'archer'}); T.append(seq,P.sanctuary(c,s)); T.lockout(seq,'wall',440); T.start(c,s,'steed',seq,M.cooldown)
  ...
  elseif T.ready(s,'wall') then T.start(c,s,'wall',P.sanctuary(c,s),440)
