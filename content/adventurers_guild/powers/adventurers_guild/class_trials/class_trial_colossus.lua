...
local function immovable(c,s)
 return {T.wait(30,function(c,s)  T.sound(c,'ITEM_ARMOR_EQUIP_NETHERITE',.5) end,
...
local function worldbreaker(c,s)
 local shape
 return {T.wait(50,function(c,s) s.charging=true; s.pressure=0; s.controlUntil=s.armed and s.tick+50 or 0; T.sound(c,'BLOCK_ANVIL_PLACE',.5) end,
  function(c,s,t)
  ...
  end,function(c,s) s.charging=false; s.armed=false; s.controlUntil=0; T.sound(c,'ENTITY_GENERIC_EXPLODE',.4); T.hit(c,shape,1.4) end),T.rest(80,1.25)}
end
return T.encounter{
