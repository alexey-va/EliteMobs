...
local function press(c,s)
 local seq={T.wait(24,function(c,s) c.boss:set_equipment('HAND','IRON_AXE',{}); T.sound(c,'ENTITY_RAVAGER_STEP',.6) end)}
 ...
 for i=1,3 do
  ...
  T.wait(18,function(c,s) arc=T.cone(c.trial:position(),T.rotate(c.trial:position(),c.trial.player:get_location(),i%2==1 and -18 or 18,4),3.5,85); T.sound(c,'BLOCK_NOTE_BLOCK_BASEDRUM',.7+i*.1) end,
   function(c,s,t) if t%4==0 then T.draw(c,arc) end end,
   function(c,s)  T.budgetHit(c,s,arc,.55,budget) end)})
 end
