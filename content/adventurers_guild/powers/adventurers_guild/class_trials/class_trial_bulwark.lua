...
local function line(c,s)
 ...
 return {T.wait(32,function(c,s) local p=c.trial:position(); guard=T.cone(p,c.trial.player:get_location(),40,100); s.guard=nil; T.sound(c,'ITEM_SHIELD_BLOCK',.5) end,
 ...
 T.wait(100,nil,function(c,s,t)
  ...
  if t%interval==0 and math.floor(t/interval)<swings then arc=T.cone(c.trial:position(),T.offset(c.trial:position(),guard.x,0,guard.z),3.2,80); T.sound(c,'BLOCK_NOTE_BLOCK_BASEDRUM',.6) end
  if t%interval<20 and t%4==0 and arc then T.draw(c,arc) end
  if t%interval==20 and arc then  T.hit(c,arc,.65); arc=nil end
 end,function(c,s) s.guard=nil end),T.rest(50)}
end
local function ground(c,s)
 local g
 return {T.wait(30,function(c,s) g=T.circle(c.trial:position(),4); T.sound(c,'BLOCK_STONE_PLACE',.6) end,
