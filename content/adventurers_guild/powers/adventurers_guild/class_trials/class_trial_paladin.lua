...
local function repulse(c,s)
  local g
  return {
    T.wait(28,function(c,s) g=T.circle(c.trial:position(),3.5); T.sound(c,'BLOCK_BELL_USE',1.1);  end,
...
local function provoke(c,s)
  local sweep
  return {
    T.wait(26,function(c,s)  T.sound(c,'ITEM_SHIELD_BLOCK',.7) end,
    ...
    T.wait(20,nil,function(c,s,t) if not s.guardBroken and s.guardPressure>=2*c.trial.matched_hit and t%4==0 then T.draw(c,sweep) end end,
      function(c,s) if not s.guardBroken and s.guardPressure>=2*c.trial.matched_hit then  T.hit(c,sweep,.65); T.sound(c,'ENTITY_PLAYER_ATTACK_SWEEP') end end),
    T.rest(40)
  }
end
return T.encounter{
 ...
 damaged=function(c,s)
   ...
   else s.guarding=false; s.guardBroken=true; c.trial:say('There. A shield has a back.') end
 end
}
