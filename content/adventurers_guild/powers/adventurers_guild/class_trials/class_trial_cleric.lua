...
local function sanctuary(c,s)
 local g
 return {T.wait(30,function(c,s) s.sanctuary=T.copy(c.trial:position()); g=T.circle(s.sanctuary,3.5); T.sound(c,'BLOCK_AMETHYST_BLOCK_CHIME',1.3);  end,
...
local function mend(c,s)
 ...
 return {T.wait(40,function(c,s)
   ...
   T.sound(c,'BLOCK_BEACON_AMBIENT',1.2)
 end,function(c,s,t)
...
return T.encounter{
 ...
 damaged=function(c,s)
   if c.trial:damaged_actor()=='boss' and s.channel then
     ...
     if s.channelDamage>=2*c.trial.matched_hit then s.channel=false; s.interrupted=true; T.sound(c,'BLOCK_GLASS_BREAK',1.5); c.trial:say('Well timed. The prayer can wait.'); s.ready.mend=s.tick+320; T.interrupt(c,s,{T.rest(40)}) end
   end
 end
}
