...
local function smite(c,s)
 local shape
 return {T.wait(32,function(c,s) local p=c.trial:position(); shape=T.lane(p,T.rotate(p,c.trial.player:get_location(),0,6),2); T.sound(c,'BLOCK_BELL_USE',1.2) end,
  function(c,s,t) if t%4==0 then T.draw(c,shape) end end,
  function(c,s)  if T.hit(c,shape,.8) then C.heal(c,s,'attendant_1',c.trial.matched_hit) end end),T.rest(60)}
end
local function banish(c,s)
 local shape
 return {T.wait(30,function(c,s) shape=T.cone(c.trial:position(),c.trial.player:get_location(),4,80); T.sound(c,'BLOCK_BELL_USE',.7);  end,
