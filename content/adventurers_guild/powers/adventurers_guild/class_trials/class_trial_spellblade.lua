...
local function cuts(c,s)
 ...
 for i=1,3 do local shape
  steps[#steps+1]=T.wait(i==1 and 30 or 16,function(c,s)  T.sound(c,'BLOCK_AMETHYST_BLOCK_CHIME',.8+i*.2) end,
   ...
   function(c,s)  T.budgetHit(c,s,shape,.4*(s.warUntil>s.tick and 1.1 or 1),budget) end)
 end
...
return T.encounter{
 ...
 choose=function(c,s)
  ...
  elseif T.ready(s,'warcasting') then T.start(c,s,'warcasting',{T.wait(28,function(c,s)  end,function(c,s,t) if t%4==0 then T.draw(c,T.circle(c.trial:position(),1.4),S.violet) end end,function(c,s) s.warUntil=s.tick+100; S.ward(c,s,'boss',1,100) end),T.rest(12)},440)
