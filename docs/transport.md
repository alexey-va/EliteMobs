...
The configured entity needs `ai: true` and must not be frozen. Transport has no fixed mount width or height limit. Preparation spawns the configured mount, measures its collision box including `scale`, and reserves the wider of mount and rider plus their combined height. Route checks and terrain loading use those dimensions. A chicken can use `entityType: CHICKEN` and `scale: 3.0` without a special transport setting. Actual terrain, world boundaries and chunk budgets still constrain travel. If the mount or rider grows after preparation, travel stops so the route can be checked again. Visible model wings may extend beyond the physical body, so the artist/admin must allow room for those too. Missing or disabled entity files and unavailable configured models reject boarding. Without LibsDisguises, the ordinary boss loader leaves the base mob visible. The journey temporarily makes the mount invulnerable and nonpersistent; it does not replace its equipment or power list.
