...
with tempfile.TemporaryDirectory(prefix='elitemobs-build-') as directory:
    ...
    if not replacements or not all(owned(n) for n in replacements):
        raise SystemExit('Unexpected compiled class outside persistence patch')
    for c in OWNERS:
    ...
    metadata = {'version':VERSION,'sourceRevision':revision,'baselineSha256':BASE_SHA256,
    ...
    with zipfile.ZipFile(a.base_jar) as base, zipfile.ZipFile(a.output,'x',zipfile.ZIP_DEFLATED) as out:
        ...
        out.writestr('META-INF/ruscrafting-patch.json',json.dumps(metadata,indent=2)+'\n')
