...
// Order is the explicit U+E500..E505 contract in CombatHud.resourceIcon.
const resourceTypes=['resolve','fury','focus','grace','mana','stamina'];
function resourceIcon(type,x,y) {
 ...
 switch(type) {
  ...
  case 'focus':
   ...
   break;
  case 'stamina':
   s=poly('2,0 8,0 8,7 10,10 10,14 0,14 0,10 3,8','#4b341f')
    +poly('3,1 7,1 7,8 9,10 9,12 1,12 1,10 4,9','#c59852')
    +rect(3,2,4,2,'#f2d48b')+rect(4,5,3,1,'#674522')
    +rect(4,7,3,1,'#674522')+rect(0,13,10,2,'#e0bd76');
   break;
  case 'grace':
...
(async()=>{
 ...
 fs.writeFileSync(path.join(__dirname, '../../src/main/resources/combat-hud-ability-icons.properties'),
  '# Generated by design/combat-hud-probe/generate.cjs\n' + sharedAbilityIcons.map(e => `${e.slot.toUpperCase()}=${e.glyph.toString(16)}`).join('\n') + '\n');
 await sharp(Buffer.from(svg(resourceTypes.length*10,15,resourceTypes.map((type,i)=>resourceIcon(type,i*10,0)).join(''))))
 ...
 const strips=[['health','#ed3f57',3],['resource_resolve','#dcad4b',3],
  ...
  ['resource_grace','#e7d7a4',3],['resource','#16cbe4',3],['xp','#74b941',2],['resource_stamina','#d6b05d',3]];
 ...
 }
 {
  ...
  providers.push(bitmap('resource_icons',7,-55,chars(0xe540,resourceTypes.length)));
